...
const TABLE_NAME = "minesweeper_scores"
...
export async function updateScore(updateData: Partial<{ wins: number; loses: number; }>): Promise<void> {
...
export async function fetchLeaderBoardScores(): Promise<LearderBoardRow[]> {
    const {data, error} = await supabase
    .from(TABLE_NAME)
    .select('username, wins, loses, score')
...
export function setupRealTimeLeaderBoard(callBackFunction: () => void) {
    ...
    .channel('minesweeper_scores_realtime')
    .on(
    'postgres_changes',
    {
        event: '*',
